...
from app.keyboards.menu import llm_menu, main_menu, nav_menu
...
async def start_llm_chat(message: Message, state: FSMContext):
    logger.info(f"User {message.from_user.id} started LLM chat")

    await state.set_state(ChatStates.chatting_with_llm)

    await message.answer(
        "🤖 Режим общения с LLM включён.\nВведите запрос.", reply_markup=llm_menu()
    )


@router.message(ChatStates.chatting_with_llm, F.text)
async def chat_with_llm(
    ...
    try:
        ...
        for chunk in chunks:
            await message.answer(chunk, reply_markup=nav_menu())
...
@router.message(
    ChatStates.chatting_with_llm, F.text.lower().in_(["меню", "exit", "/exit"])
)
async def exit_llm_chat(message: Message, state: FSMContext):
    await state.clear()
